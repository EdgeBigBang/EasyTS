...
if __name__ == "__main__":
    ...
    y1 = np.random.normal(mean1, std1, size)

    # 生成第二个随机序列，使其与第一个序列相近
    mean2 = mean1 + np.random.uniform(-5, 5)  # 在第一个均值的基础上加入小的随机偏移
    std2 = std1 + np.random.uniform(0, 2)  # 在第一个标准差的基础上加入小的随机偏移

    y2 = np.random.normal(mean2, std2, size)
